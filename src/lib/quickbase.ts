...
async function qbQuery(tableId: string, select: number[], where: string, top = 500) {
...
export async function getInstalls(fromDate: string, toDate: string): Promise<QBInstall[]> {
  const data = await qbQuery(
    QB_PROJECTS_TABLE,
    INSTALL_FIELDS,
