...
import { teamIdToQBOffice, getActiveOffices } from '@/lib/config';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const weekStart = body.weekStart || format(startOfWeek(new Date(), { weekStartsOn: 0 }), 'yyyy-MM-dd');
    const weekEnd = body.weekEnd || format(endOfWeek(new Date(), { weekStartsOn: 0 }), 'yyyy-MM-dd');
    ...
    const snapshot = {
      ...
      data: { leaderboards: leaderboards?.length, users: users?.length, sales: sales?.length, appointments: appointments?.length },
    };

    const { error } = await supabaseAdmin.from('weekly_snapshots').upsert(snapshot, { onConflict: 'week_start' });
