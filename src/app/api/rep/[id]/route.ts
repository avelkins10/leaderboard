...
export async function GET(
  ...
  try {
    ...
    // Quality stats from Supabase (already fetched in parallel above)
    let qualityStats = null;
    if (qualityRows && qualityRows.length > 0) {
      const total = qualityRows.length;
      const withPowerBill = qualityRows.filter(
        (a) => a.has_power_bill === true,
      ).length;
      const within48hrs = qualityRows.filter(
        (a) =>
          a.hours_to_appointment != null &&
          a.hours_to_appointment > 0 &&
          a.hours_to_appointment <= 48,
      ).length;
      const threeStarCount = qualityRows.filter(
        (a) => a.is_quality === true,
      ).length;
      const twoStarCount = qualityRows.filter(
        (a) =>
    ...
    {
      const closerSales = repSales.filter(
        (s) =>
          s.closerRepCardId === String(userId) ||
          s.closerName?.toLowerCase().includes(fullName.toLowerCase()),
      );
      if (closerSales.length > 0) {
        ...
        const totalKw =
          Math.round(
            closerSales.reduce((sum, s) => sum + (s.systemSizeKw || 0), 0) *
              100,
          ) / 100;
