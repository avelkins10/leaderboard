                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={chartData} layout="vertical" margin={{ left: 0, right: 16, top: 4, bottom: 4 }}>
                      ...
                      <Bar dataKey="deals" radius={[0, 4, 4, 0]} maxBarSize={18}>
                        {chartData.map((_, i) => (
                          <Cell key={i} fill={i === 0 ? C.primary : C.barDefault} fillOpacity={i === 0 ? 1 : 0.5} />
                        ))}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                    ...
                    .map((c, i) => {
                      ...
                      return (
                        <Link key={c.userId} href={`/rep/${c.userId}`}
                          className="group flex items-center justify-between px-6 py-3 transition-colors hover:bg-secondary/40">
                          ...
                          <div className="flex items-center gap-4 shrink-0">
                            ...
                            {(c.CLOS || 0) > (c.qbCloses || 0) && (
                              <span className="text-2xs font-mono text-warning/70">({c.CLOS})</span>
                            )}
              ...
              <table className="w-full">
                ...
                <tbody className="text-[13px]">
                  {officeEntries.map(([office, d]: [string, any]) => {
                    ...
                    return (
                      <tr key={office} className={`border-b border-border/30 transition-colors hover:bg-secondary/30 ${qbCloses === 0 ? 'opacity-25' : ''}`}>
                        ...
                        <td className="py-3.5 px-3 text-right">
                          {totalSits > 0 ? <StatusBadge value={Math.round(sitClose)} good={35} ok={25} /> : <span className="text-muted-foreground/15 font-mono">--</span>}
                        </td>
                      </tr>
                    );
                  })}
              ...
              <div className="grid grid-cols-2 gap-2.5">
                {Object.entries(data.salesByCloser)
                  ...
                  .map(([name, d]: [string, any]) => (
                    <div key={name} className="rounded-lg border border-border/30 bg-secondary/20 p-4 transition-all hover:bg-secondary/40 hover:border-border/60">
