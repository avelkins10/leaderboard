...
```
hours_to_appointment = (appointment_time - lead creation time (contact.createdAt)) / (1000 * 60 * 60)
```
...
This tells the story of _why_ they have low sit rate.
...
- **Computed by**: `fetchScorecard()` in `data.ts` — queries Supabase `appointments.star_rating` grouped by `setter_id`
- **Formula**: `sum(star_rating) / count(star_rating)` for each setter's appointments in the date range
- **Used on**: Setter leaderboard, office setter accountability

### PPW Outlier Filtering

- Deals with PPW outside 0.5–8.0 range are excluded from average PPW calculations
- Defined in `data.ts`: `isValidPpw(ppw)` returns `ppw >= 0.5 && ppw <= 8.0`
- Prevents bad data from skewing averages

### Recruit Badge

- Reps whose first name starts with "R - " are flagged as recruits
- `cleanRepName()` strips the "R - " prefix for display
- `isRecruit()` returns true/false — used to show a blue "R" badge on leaderboards
